 ...
 * Date: 2015-03-18
 * License: CC0
 * Source: https://software.intel.com/sites/landingpage/intrinsics_guide/
...
typedef __m256i mi;
template<class T>
mi load_unaligned(T& value) {
	return _mm256_loadu_si256(reinterpret_cast<const mi*>(&value));
}
...
int sumi32(mi m) { union {int v[8]; mi m;} u; u.m = m;
	int ret = 0; for (int i = 0; i < (8); ++i) ret += u.v[i]; return ret; }
...
ll example_filtered_dot_product(int n, short* a, short* b) {
	int i = 0; ll r = 0;
	mi zero = _mm256_setzero_si256(), acc = zero;
	while (i + 16 <= n) {
		mi va = load_unaligned(a[i]), vb = load_unaligned(b[i]); i += 16;
