...
ll sqrt(ll a, ll p) {
	a %= p; if (a < 0) a += p;
	if (a == 0) return 0;
	assert(mod_pow(a, (p-1)/2, p) == 1); // else no solution
	...
	while (mod_pow(n, (p - 1) / 2, p) != p - 1) ++n;
	ll x = mod_pow(a, (s + 1) / 2, p);
	ll b = mod_pow(a, s, p), g = mod_pow(n, s, p);
	for (;; r = m) {
		ll t = b;
		for (m = 0; m < r && t != 1; ++m)
			t = t * t % p;
		if (m == 0) return x;
		ll gs = mod_pow(g, 1LL << (r - m - 1), p);
		g = gs * gs % p;
		x = x * gs % p;
		b = b * g % p;
	}
