 ...
 * k is zero-based and nonnegative; transition must be nonempty and sequence
 * must contain at least n initial terms. transition[0] multiplies the most
 * recent term. Define MOD yourself (prime not required), normalize inputs to
 * $[0,MOD)$, and ensure multiply-adds fit in ll (or widen to \_\_int128).
 * Use the same MOD as BerlekampMassey if obtaining coefficients there.
...
ll linear_rec(vector<ll> sequence, vector<ll> transition, ll k) {
	...
	auto combine = [&](vector<ll> a, vector<ll> b) {
		...
		// Reduce high powers using the recurrence coefficients.
